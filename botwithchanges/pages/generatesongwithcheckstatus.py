...
class SoundOfMeme:
    def __init__(self):
        ...
        self.base_song_status_url = "https://testapi.soundofmeme.com/songs/"
    ...
    def fetch_slugs_for_uploaded_ids(self, uploaded_ids):
        ...
        slugs = []
        page = 1
        print("song ids from ", uploaded_ids)
        while True:
            try:
    ...
    def check_song_status(self, song_ids):
        """
        Check the status of the given song IDs.

        :param song_ids: List of song IDs to check.
        :return: Dictionary containing the status and details of the songs.
        """
        if not self.access_token:
            print("Access token is missing!")
            return None
        ...
        try:
            ...
            url = f"{self.base_song_status_url}?song_ids={ids_param}"
            headers = {"Authorization": f"Bearer {self.access_token}"}
            ...
            print(f"API song status check failed: {e}")
            return None
